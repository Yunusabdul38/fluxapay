        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between p-4 bg-secondary/50 rounded-xl border border-border">
                <div>
                    ...
                    <div className="flex items-baseline gap-2">
                        ...
                        <span className="text-sm font-semibold text-muted-foreground">{payment.currency}</span>
                    </div>
                </div>
                <div className="flex flex-col items-end">
                    <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-background border border-border shadow-sm">
                        {getStatusIcon(payment.status)}
                        <span className="capitalize font-medium text-sm">{payment.status}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div className="space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground font-mono">{payment.merchantId}</p>
                </div>
                <div className="space-y-1">
                ...
                {payment.settlementId && (
                    <div className="space-y-1">
                        ...
                        <p className="font-mono text-primary cursor-pointer hover:underline">{payment.settlementId}</p>
                    </div>
                )}
            </div>
            ...
            <div className="pt-4 border-t border-border">
                <h4 className="font-semibold mb-4 text-sm uppercase tracking-wider text-muted-foreground">Event Timeline</h4>
                <div className="space-y-6 relative ml-2">
                    ...
                    {payment.events.map((event, idx) => (
                        <div key={event.id} className="relative flex items-start gap-4">
                            {/* Dot */}
                            <div className={`mt-1 h-4 w-4 rounded-full border-2 border-background shadow-sm z-10 
            ...
            {payment.networkTxHash && (
                <div className="pt-4 mt-2">
                    <Button className="w-full gap-2" variant="outline" asChild>
                        <a href={`${STELLAR_EXPLORER_BASE}/${payment.networkTxHash}`} target="_blank" rel="noreferrer">
