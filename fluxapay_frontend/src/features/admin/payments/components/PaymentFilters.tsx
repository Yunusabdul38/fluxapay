...
export function PaymentFilters({ filters, onFilterChange }: PaymentFiltersProps) {
    ...
    const clearFilters = () => {
        setLocalMerchant('');
        onFilterChange({ merchant: '', status: 'all', dateRange: { from: undefined, to: undefined } });
    };
                ...
                <div className="h-10 border border-input rounded-md flex items-center bg-background px-2 overflow-hidden w-auto">
                    ...
                    <input
                        ...
                        onChange={(e) => onFilterChange({
                            ...filters,
                            dateRange: { ...filters.dateRange, from: e.target.value ? new Date(e.target.value) : undefined, to: filters.dateRange?.to }
                        })}
                    />
                    ...
                    <input
                        ...
                        onChange={(e) => onFilterChange({
                            ...filters,
                            dateRange: { ...filters.dateRange, to: e.target.value ? new Date(e.target.value) : undefined, from: filters.dateRange?.from }
                        })}
                    />
                </div>

                {(filters.merchant || (filters.status && filters.status !== 'all') || filters.dateRange?.from) && (
